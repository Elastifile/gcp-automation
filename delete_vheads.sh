...
# jsonValue function based on code from https://gist.github.com/cjus/1047794 by itstayyab
function jsonValue() {
KEY=$1
 /
...
function create_instances {
  ...
  curl -k -b ${SESSION_FILE} -H "Content-Type: application/json" -X POST -d '{"instances":'$1',"async":true}' https://${EMS_ADDRESS}/api/hosts/delete_instances >> ${LOG} 2>&1
}
