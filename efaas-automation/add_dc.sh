...
# creating an array for the acl access rights
  ...
  echo -e "Configure filesystem.." | tee -a ${LOG}
  if (( $i == 1 )); then
	result=$(curl -k -X POST "$EFAAS_END_POINT/api/v2/projects/$PROJECT/instances/$NAME/filesystem" -H "accept: application/json" -H "Content-Type: application/json" -d "{ \"name\": \"$DC\", \"description\": \"$DC_DESCRIPTION\", \"quotaType\": \"$QUOTA_TYPE\", \"hardQuota\": $HARD_QUOTA, \"snapshot\": { \"enable\": $SNAPSHOT, \"schedule\": \"$SNAPSHOT_SCHEDULER\", \"retention\": $SNAPSHOT_RETENTION }, \"accessors\": { \"items\": [ { \"sourceRange\": \"${acl_range_array[0]}\", \"accessRights\": \"${acl_rights_array[0]}\" } ], \"fingerprint\": \"$fingerprint\" }}" -H "$token")
  elif (( $i == 2 )); then
	result=$(curl -k -X POST "$EFAAS_END_POINT/api/v2/projects/$PROJECT/instances/$NAME/filesystem" -H "accept: application/json" -H "Content-Type: application/json" -d "{ \"name\": \"$DC\", \"description\": \"$DC_DESCRIPTION\", \"quotaType\": \"$QUOTA_TYPE\", \"hardQuota\": $HARD_QUOTA, \"snapshot\": { \"enable\": $SNAPSHOT, \"schedule\": \"$SNAPSHOT_SCHEDULER\", \"retention\": $SNAPSHOT_RETENTION }, \"accessors\": { \"items\": [ { \"sourceRange\": \"${acl_range_array[0]}\", \"accessRights\": \"${acl_rights_array[0]}\" }, { \"sourceRange\": \"${acl_range_array[1]}\", \"accessRights\": \"${acl_rights_array[1]}\" } ], \"fingerprint\": \"$fingerprint\" }}" -H "$token")  
  elif (( $i == 3 )); then
  	result=$(curl -k -X POST "$EFAAS_END_POINT/api/v2/projects/$PROJECT/instances/$NAME/filesystem" -H "accept: application/json" -H "Content-Type: application/json" -d "{ \"name\": \"$DC\", \"description\": \"$DC_DESCRIPTION\", \"quotaType\": \"$QUOTA_TYPE\", \"hardQuota\": $HARD_QUOTA, \"snapshot\": { \"enable\": $SNAPSHOT, \"schedule\": \"$SNAPSHOT_SCHEDULER\", \"retention\": $SNAPSHOT_RETENTION }, \"accessors\": { \"items\": [ { \"sourceRange\": \"${acl_range_array[0]}\", \"accessRights\": \"${acl_rights_array[0]}\" }, { \"sourceRange\": \"${acl_range_array[1]}\", \"accessRights\": \"${acl_rights_array[1]}\" }, { \"sourceRange\": \"${acl_range_array[2]}\", \"accessRights\": \"${acl_rights_array[2]}\" } ], \"fingerprint\": \"$fingerprint\" }}" -H "$token")
  elif (( $i == 4 )); then
